...
**The Detail Page Rule.** Row tap navigates to [`/scrap/:id`](src/pages/ScrapDetail.tsx). Full page in the app chrome (Header/Footer), not an auth/settings sheet. Back is a Library icon (`auth-back-btn` + IconTip **책장으로**). Share / bookmark / read / remind sit **inside** `dashboard-panel` under the title meta. Peel is centered alone under the tags. Neighbor prev/next previews sit **below** the panel (newest-first shelf order). Arrow keys and Escape return to shelf. Peel uses the centered AppDialog, then deletes and returns home. List rows use a magnet Lucide `Bookmark` (not a star glyph) next to the unread dot. Loading the detail list reuses `AuthWaiting` (circular spinner). Use dashboard-door / dashboard-panel paper language — never login-wall floating sheet.
