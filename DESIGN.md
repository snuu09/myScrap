...
**The Look Axis Rule.** A third prefs axis `data-look` = `fridge` | `library` (default fridge). Library remaps the same CSS tokens (`--color-enamel`, `--color-paper`, shadows) toward a warmer shelf tone. It does not replace palette (magnets) or invent a second component kit. No 3D bookcase mesh; at most a soft binding edge on cards. When Look is **library**, UI copy swaps unit nouns (KO 조각→페이지, 떼어내기→책장에서 빼기; EN scrap→page, peel→take off the shelf) via `useT` / `t(..., look)`. Fridge Look keeps the original capture-box wording.
...
**The List Tools Rule.** Search, type chips, and day chip live in one **list-tools** paper panel (18px radius, scrap shadow). Label is caption/muted. Search is a 48px capsule. Chips are 34px enamel pills; active chip is magnet fill. Clear-filters link sits in the panel header when any filter is on. A layout segment (리스트 / 그리드 / 갤러리) sits in the same head row; preference persists as `mybrary.shelfLayout`. Ad slot and list body are separate sections below.
