...
**The Detail Page Rule.** Row tap navigates to [`/scrap/:id`](src/pages/ScrapDetail.tsx). Full page in the app chrome (Header/Footer), not an auth/settings sheet. The header keeps the logo. Off the shelf, an arrow sits immediately left of the logo and returns (`header-back` + IconTip). Actions sit **inside** `dashboard-panel` under the title meta, in three groups: edit and peel, then AI and share, then bookmark, read, and remind. Edit mode (pencil) changes title, memo, and tags only; Escape cancels edit. Share appears only when the scrap has an external URL. Tag chips show how many scraps use that tag. If a prior edit or AI write exists, a history list can compare, restore, or delete that snapshot. Related pages, scored from shared tags, domain, type, and title words, sit in a horizontal row with no cap. Each card is a short cover (about 4.6rem) plus a title clamped to two lines. If the stored title is only the domain or URL, the card uses a short OG description, then a shortened `og.title`. The stage fills the height under the header. Neighbor prev/next peeks sit beside the panel. On wide screens they use the leftover width at the same height as the page, drawn faded so the paper shows through, and sharpen on hover. Below 960px they collapse to chevrons; a fine pointer hover shows a small cover and title, and touch does not. A peek click starts the spine-hinged turn in that same frame. The incoming layer is the next page's title, meta, and a fixed-ratio cover, not a stretched image. When the leaf has left, that preview continues in place and the next route does not fade in. Reduced motion skips the turn. The peek image preloads the neighbor cover. Do not also show a neighbor row under the panel. Arrow keys and Escape return to shelf (Escape exits edit first). Peel uses the centered AppDialog, then deletes and returns home. List rows show a magnet **corner bookmark ribbon** when bookmarked (not an inline glyph). Type chips in list-tools hide types with count 0. Loading the detail list reuses `AuthWaiting` (circular spinner). Use dashboard-door / dashboard-panel paper language — never login-wall floating sheet.
...
**The Dashboard Rule.** Route `/dashboard`, header caption **대시보드** / Dashboard when signed in. Centered **dashboard-door** column max 40rem with **dashboard-panel** paper cards (same tokens as list-tools): plan via `PlanUsageBlock` (tier, D-day, **trial end date**, storage text + linear gauge; the operator-upgrade sentence is hidden here and kept in settings), type and tag rows (name, count, row menu), recent 10 timeline, top-7 days. A type or tag row opens search. The tag menu renames or deletes that tag on the scraps that use it. Type rows can add, rename, or delete a name. The add field has a visible cancel as well as Escape. Rows that open search or a scrap use the same hover fill and weak shadow as day cells. The row menu unfolds from the top right and folds closed before it unmounts. A new type row unfolds under the header. Rename folds the field before the row returns. Delete folds the row, then updates the catalog; a failed save restores the row. Rename rewrites that type on the scraps that use it. Delete does not remove the scraps; it clears the type. The six built-in names start visible even at zero, and a chevron below expands a long type or tag list. A day row opens `/search?day=`. Empty sections use compact shelf-empty. The header logo stays; the arrow to its left returns to the shelf. Not a second home.
