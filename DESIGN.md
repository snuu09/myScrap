...
**The List Tools Rule.** Search, type chips, and day chip live in one **list-tools** paper panel (18px radius, scrap shadow). Label is caption/muted. Search is a 48px capsule. Chips are 34px enamel pills; active chip is magnet fill. Clear-filters link sits in the panel header when any filter is on. A layout segment (리스트 / 갤러리) sits in the same head row; preference persists as `mybrary.shelfLayout`. Ad slot and list body are separate sections below.
