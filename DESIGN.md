...
**The Look Axis Rule.** A third prefs axis `data-look` = `fridge` | `library` (default library). Library remaps the same CSS tokens (`--color-enamel`, `--color-paper`, shadows) toward a warmer shelf tone. It does not replace palette (magnets) or invent a second component kit. No 3D bookcase mesh; binding spine, soft shelf rails, and paper shadow stay CSS-only on cards. When Look is **library**, UI copy swaps unit nouns (KO 조각→페이지, 떼어내기→책장에서 빼기; EN scrap→page, peel→take off the shelf) via `useT` / `t(..., look)`. Fridge Look keeps the original capture-box wording.
...
**The List Tools Rule.** Search is the header capsule → `/search` with no site header (search bar, back, results; footer stays so 이용약관 still has a path). Autofocus field, placeholder **검색어를 입력해주세요.**, type books, paper tag chips. More than eight tags collapse behind a chevron (`aria-label` keeps 펼치기/접기). **일자별** is a calendar icon in the search bar (magnet when a day is selected or the panel is open). Shelf list-tools keeps the layout segment. Gallery is the default; list and accordion are options. Accordion groups the filtered window by scrap type (name and count). Several sections can be open at once, and they start open. Cards inside a group reuse the list row. Preference persists as `mybrary.shelfLayout`. Type books sit above list-tools. Ad slot and list body stay separate sections below. Lists mount a page of cards, then more when the sentinel nears the viewport. Do not mount the whole shelf at once.
