...
- Type chips, search, and **일자별** day filter (AND). Peel from the list or detail page. Detail actions: open link, share, bookmark, read/unread, remind (foreground Notification once). List bookmark uses the same Lucide bookmark mark as detail (unread stays a magnet dot).
