...
type Props = { scraps: Scrap[]; onScrapsChange: (next: Scrap[]) => void; kind: "types" | "tags" };
...
export function DashboardEditor({ scraps, onScrapsChange, kind }: Props) {
  const { lang } = usePrefs();
  const t = useT();
  const { user } = useAuth();
  ...
  const showTypes = kind === "types";

  return (
    <div className="dashboard-door">
      <div className="dashboard-head">
        <h1 className="dashboard-title">{t(showTypes ? "manageTypes" : "manageTags")}</h1>
      </div>
      ...
      <section className="dashboard-panel" aria-label={t("manageTypes")}>
            ...
            {shownTypes.map((type) =>
              editingType === type ? (
                ...
                >
                  ...
                  >
                    <button type="button" role="menuitem" onClick={() => navigate("/search?type=" + encodeURIComponent(type))}>
                      {t("viewInSearch")}
                    </button>
                    <button
                      type="button"
        ...
        {typeRows.length > 6 ? (
          <GlassCluster className="liquid-hit liquid-hit--wide">
            <button type="button" className="dashboard-expand" aria-expanded={typesOpen} onClick={() => setTypesOpen((open) => !open)}>
      ...
      <section className="dashboard-panel" aria-label={t("manageTags")}>
            ...
            {shownTags.map(([tag, count]) =>
              editingTag === tag ? (
                ...
                >
                  ...
                  >
                    <button type="button" role="menuitem" onClick={() => navigate("/search?tag=" + encodeURIComponent(tag))}>
                      {t("viewInSearch")}
                    </button>
                    <button
                      type="button"
        ...
        {stats.byTag.length > 8 ? (
          <GlassCluster className="liquid-hit liquid-hit--wide">
            <button type="button" className="dashboard-expand" aria-expanded={tagsOpen} onClick={() => setTagsOpen((open) => !open)}>
