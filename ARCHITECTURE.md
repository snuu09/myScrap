...
| OG | [`src/lib/og.ts`](src/lib/og.ts), [`supabase/functions/og-preview`](supabase/functions/og-preview) | URL preview after classify; Detail lazy backfill. Signed-in clients prefer `og-preview` (OG + ephemeral metaDescription/excerpt for classify). Microlink / HTML scrape are fallbacks. |

## Data flow

1. **Entry.** Intro is public. **책장을 연다** opens email sign in / sign up. A session paints the shelf.
2. **Capture.** Paste/drop/`+`. Account files upload to `scrap-media` first so the Supabase function `analyze` (user JWT) can classify with Claude (`summary` + `analysis`, language from prefs); images may use vision. For links, `og-preview` supplies OG fields plus a short page excerpt (ephemeral, not stored) so Claude grounds summary/analysis in page text rather than inventing from the URL alone. Failure uses the local tagger. Discarding the classify draft removes any pending Storage object. Guest media stays on-device. Detail **AI 분석** re-runs the same endpoint and stores summary in `body` / analysis in `preview_text`.
