...
| OG | [`src/lib/og.ts`](src/lib/og.ts), [`supabase/functions/og-preview`](supabase/functions/og-preview) | URL preview after classify; Detail lazy backfill. Signed-in clients prefer `og-preview` (OG + metaDescription/excerpt up to 6k for classify). Microlink / HTML scrape are fallbacks. Readable excerpt is stored as `source_text`. |

## Data flow

1. **Entry.** Intro is public. **책장을 연다** opens email sign in / sign up. A session paints the shelf.
2. **Capture.** Paste/drop/`+`. Account files upload to `scrap-media` first so the Supabase function `analyze` (user JWT) can classify with Claude (`summary` + `analysis`, language from prefs); images may use vision. For links, `og-preview` supplies OG fields plus a page excerpt (up to 6k) so Claude grounds summary/analysis; the excerpt is stored as `source_text` for draft/detail reading (full HTML is not stored). Plain text keeps the paste in `source_text` while `body` holds the short summary. Failure uses the local tagger. Discarding the classify draft removes any pending Storage object. Guest media stays on-device. Detail **AI 분석** re-runs the same endpoint and stores summary in `body` / analysis in `preview_text`.
